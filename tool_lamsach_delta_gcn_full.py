...
def build_delta_gcn_payload(ttdk):
    """
    Build payload delta cho endpoint LamSachDuLieuAjax/CapNhatThongKePhanLoaiThuaDatChiTiet.
    Chỉ cập nhật 2 trường của GCN bị thiếu: soVaoSo và ngayVaoSo.
    Không đưa nguồn gốc/thửa đất vào delta này.
    """
    thdk = ttdk.get("TinhHinhDangKy") or {}
    tid = thdk.get("tinhHinhDangKyId")
    data = {}

    for gcn in ttdk.get("ListGiayChungNhan") or []:
        changed_fields = gcn.get("_delta_gcn_changed_fields") or []
        if not changed_fields:
            continue

        gid = gcn.get("giayChungNhanId")
        ver = gcn.get("version")
        if not tid or not gid or not ver:
            continue

        prefix = f"TINHHINHDANGKY.{tid}|GIAYCHUNGNHAN.{gid}_{ver}"

        if "soVaoSo" in changed_fields:
            data[f"{prefix}|soVaoSo"] = gcn.get("soVaoSo") or SO_VAO_SO_DEFAULT

        if "ngayVaoSo" in changed_fields:
            data[f"{prefix}|ngayVaoSo"] = gcn.get("ngayVaoSo")
...
def api_update_thong_tin_dang_ky(session, tinh_hinh_dang_ky_id, ngay_dang_ky_lan_dau,
    ...
    # 1) Cập nhật riêng số/ngày vào sổ GCN bằng endpoint delta làm sạch dữ liệu.
    #    Endpoint này chỉ nhận data dạng key-path, không gọi UpdateGiayChungNhan.
    ...
    # 2) Update từng cá nhân trước để maSoDinhDanh/maDinhDanhCaNhan ăn vào node chủ.
    ...
    # 3) Update từng thửa đất trước để nguồn gốc theo từng mục đích ăn vào node tài sản.
    ...
    # 4) Không gọi UpdateGiayChungNhan.
    #    Số/ngày vào sổ GCN đã được cập nhật bằng delta ở bước trên.
    #    Các thông tin còn lại gửi UpdateThongTinDangKy tổng.
