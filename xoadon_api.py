...
# True = chỉ kiểm tra, không update thật (áp dụng cho chế độ Cập nhật HSQ).
DRY_RUN = False
...
MODE_CAP_NHAT_HSQ = "cap_nhat_hsq"
...
def doc_danh_sach_cap_nhat_hsq(file_path: str) -> list[dict[str, Any]]:
    """
    Đọc Excel cho chế độ Cập nhật HSQ — header CỐ ĐỊNH ở hàng 1, tên cột đúng
    (không phân biệt hoa/thường): soto, sothua, loaidat, tenfile, sogcn.
    Cùng quy ước với doc_excel() trong cap_nhat_hosoquet.py.
    """
    extension = Path(file_path).suffix.lower()
    if extension not in {".xlsx", ".xlsm"}:
        raise ValueError("Chỉ hỗ trợ file Excel .xlsx hoặc .xlsm.")
    ...
    try:
        ...
        if missing:
            raise ValueError(f"Thiếu cột trong Excel: {', '.join(missing)}")

        rows: list[dict[str, Any]] = []

        for row_index in range(2, worksheet.max_row + 1):
            so_to    = worksheet.cell(row=row_index, column=headers["soto"]).value
            so_thua  = worksheet.cell(row=row_index, column=headers["sothua"]).value
            loai_dat = worksheet.cell(row=row_index, column=headers["loaidat"]).value
            ten_file = worksheet.cell(row=row_index, column=headers["tenfile"]).value
            so_gcn   = worksheet.cell(row=row_index, column=headers["sogcn"]).value

            if not so_to and not so_thua and not loai_dat and not ten_file and not so_gcn:
                continue

            rows.append(
                {
                    "excel_row": row_index,
                    "soto":    chuan_hoa_gia_tri_excel(so_to),
                    "sothua":  chuan_hoa_gia_tri_excel(so_thua),
                    "loaidat": chuan_hoa_gia_tri_excel(loai_dat),
                    "tenfile": chuan_hoa_gia_tri_excel(ten_file),
                    "sogcn":   chuan_hoa_gia_tri_excel(so_gcn),
                }
            )
...
class ExcelCapNhatHsqWriter:
    """Ghi kết quả cho chế độ Cập nhật HSQ (khác cột với ExcelResultWriter của xóa đơn)."""
    ...
    def _setup_sheet(self) -> None:
        self.worksheet.append(OUTPUT_HEADERS_CAP_NHAT_HSQ)
        ...
        self.worksheet.freeze_panes = "A2"
        self.worksheet.auto_filter.ref = f"A1:{get_column_letter(len(OUTPUT_HEADERS_CAP_NHAT_HSQ))}1"
        self.worksheet.row_dimensions[1].height = 30
...
class MplisClient:
    ...
    def api_update_hosoquet_exist_file(
        ...
        """Upload file PDF thật thay cho file CHUACOGIAY hiện có trong HSQ."""
        ...
        ho_so_quet = {
            "hoSoQuetId":       safe_int(hoso.get("hoSoQuetId") or hoso.get("Title")),
            ...
            "Id":               hoso.get("Id"),
            "Title":            str(hoso.get("Title") or hoso.get("hoSoQuetId")),
            "Name":             hoso.get("Name"),
            "Path":             hoso.get("Path"),
    ...
    def api_update_thong_tin_dang_ky(self, payload: dict[str, Any], debug_key: str = "") -> str:
        ...
        for mode, body in payload_variants:
            try:
                ...
                with open(
                    debug_dir / f"UpdateThongTinDangKy_{mode}{suffix}.json",
                    "w", encoding="utf-8"
                ) as f:
...
def tim_hoso_chuacogiay_trong_chi_tiet(detail: dict[str, Any]) -> dict[str, Any] | None:
    """
    detail: kết quả lay_chi_tiet_don() — ListHoSoQuet nằm ở TOP-LEVEL (khác với
    SearchHoSoQuet của cap_nhat_hosoquet.py vốn nested trong "data[].ListHoSoQuet").

    Ưu tiên hoso đang có file mô tả CHUACOGIAY; nếu không còn file nào như vậy,
    fallback lấy hoso đầu tiên (file=None) để vẫn còn tinhHinhDangKyId phục vụ
    cập nhật thông tin đăng ký, dù không có gì để sửa mô tả/loại HSQ.
    """
    list_ho_so_quet = detail.get("ListHoSoQuet")
...
def build_payload_update_ttdk(
    ...
    (lay_chi_tiet_don) — KHÔNG cần GET lại lần nữa (khác cap_nhat_hosoquet.py,
    vốn phải GET riêng vì chỉ có tinhHinhDangKyId từ SearchHoSoQuet).
    Chỉ sửa đúng coQuyenQuanLy, thoiDiemDangKyLanDau, và chủ (nếu có chu_id).
    """
    payload = {
...
def xu_ly_mot_don_cap_nhat_hsq(
    client: MplisClient,
    ...
    """
    Xử lý 1 đơn (1 tinhHinhDangKyId) đã tìm được: lấy chi tiết (kèm HSQ), tìm
    file CHUACOGIAY, đẩy file thật (hoặc chỉ sửa metadata nếu không đẩy), rồi
    cập nhật thông tin đăng ký/chủ. Trả về 1 dict kết quả cho 1 dòng Excel output.
    """
    result: dict[str, Any] = {
        "soto": so_to,
        "sothua": so_thua,
    ...
    found = tim_hoso_chuacogiay_trong_chi_tiet(detail)
    if found is None:
    ...
    result["hosoquetid"] = hoso.get("hoSoQuetId") or hoso.get("Title") or ""
    ...
    try:
        if day_hsq:
            file_path = (
                ten_file if os.path.isabs(ten_file) else os.path.join(folder_upload, ten_file)
            )
            client.api_update_hosoquet_exist_file(
                file_path=file_path, hoso=hoso, mo_ta_moi=mo_ta_moi, loai_ho_so_quet=loai_ho_so_quet
            )
        else:
            client.api_update_hosoquet_metadata_only(
                hoso=hoso, target_file=target_file, mo_ta_moi=mo_ta_moi, loai_ho_so_quet=loai_ho_so_quet
            )
    except Exception as exc:
    ...
    result["ghi_chu"] = "Đã cập nhật HSQ + thông tin đăng ký/chủ."
    return result
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        # ---- Tuỳ chọn riêng cho chế độ Cập nhật hồ sơ quét ----
        self.var_ngay_dk = tk.StringVar()
        self.var_chu_id = tk.StringVar()
        self.var_day_hsq_capnhat = tk.BooleanVar(value=True)
        self.var_folder_upload = tk.StringVar()
        self.var_dry_run_capnhat = tk.BooleanVar(value=False)
    ...
    def _build_ui(self) -> None:
        ...
        self.rad_cap_nhat_hsq = ttk.Radiobutton(
            mode_frame,
            text="Cập nhật hồ sơ quét (CHUACOGIAY → thật) + thông tin đăng ký/chủ",
            variable=self.var_mode,
            value=MODE_CAP_NHAT_HSQ,
            command=self._on_mode_change,
        )
        self.rad_cap_nhat_hsq.pack(anchor="w", padx=5, pady=1)

        self.mode_radios = [
            self.rad_chi_kiem_tra,
            self.rad_kiem_tra_xoa,
            self.rad_xoa_thang,
            self.rad_cap_nhat_hsq,
        ]
        ...
        # ── Tuỳ chọn riêng cho chế độ "Cập nhật hồ sơ quét" (ẩn/hiện theo mode) ──
        self.frame_cap_nhat_hsq = ttk.LabelFrame(
            self.root, text="Tuỳ chọn Cập nhật hồ sơ quét", padding=8
        )

        ttk.Label(self.frame_cap_nhat_hsq, text="Ngày ĐK lần đầu:").grid(
            row=0, column=0, sticky="w", padx=5, pady=3
        )
        self.ent_ngay_dk = ttk.Entry(self.frame_cap_nhat_hsq, textvariable=self.var_ngay_dk, width=14)
        self.ent_ngay_dk.grid(row=0, column=1, sticky="w", padx=5, pady=3)
        ttk.Label(self.frame_cap_nhat_hsq, text="dd/mm/yyyy", foreground="gray").grid(
            row=0, column=2, sticky="w"
        )

        ttk.Label(self.frame_cap_nhat_hsq, text="ID thông tin chủ:").grid(
            row=0, column=3, sticky="w", padx=5, pady=3
        )
        self.ent_chu_id = ttk.Entry(self.frame_cap_nhat_hsq, textvariable=self.var_chu_id, width=16)
        self.ent_chu_id.grid(row=0, column=4, sticky="w", padx=5, pady=3)
        ttk.Label(
            self.frame_cap_nhat_hsq, text="(để trống nếu không đổi chủ)", foreground="gray"
        ).grid(row=0, column=5, sticky="w", padx=5, pady=3)

        ttk.Label(self.frame_cap_nhat_hsq, text="Loại hồ sơ quét:").grid(
            row=1, column=0, sticky="w", padx=5, pady=3
        )
        self.cbo_loai_hsq = ttk.Combobox(
            self.frame_cap_nhat_hsq,
        ...
        self.chk_day_hsq_capnhat = ttk.Checkbutton(
            self.frame_cap_nhat_hsq,
            text="Đẩy hồ sơ quét (upload file PDF)",
            variable=self.var_day_hsq_capnhat,
            command=self._on_toggle_day_hsq_capnhat,
        )
        self.chk_day_hsq_capnhat.grid(row=1, column=3, columnspan=2, sticky="w", padx=5, pady=3)

        self.chk_dry_run_capnhat = ttk.Checkbutton(
            self.frame_cap_nhat_hsq,
            text="DRY RUN (chỉ kiểm tra, không update)",
            variable=self.var_dry_run_capnhat,
        )
        self.chk_dry_run_capnhat.grid(row=1, column=5, sticky="w", padx=5, pady=3)

        ttk.Label(self.frame_cap_nhat_hsq, text="Folder PDF:").grid(
            row=2, column=0, sticky="w", padx=5, pady=3
        )
        self.ent_folder_upload = ttk.Entry(
            self.frame_cap_nhat_hsq, textvariable=self.var_folder_upload, width=60
        )
        self.ent_folder_upload.grid(row=2, column=1, columnspan=4, sticky="ew", padx=5, pady=3)
        self.btn_browse_folder_upload = ttk.Button(
            self.frame_cap_nhat_hsq, text="Duyệt...", command=self.chon_folder_upload
        )
        self.btn_browse_folder_upload.grid(row=2, column=5, sticky="w", padx=5, pady=3)

        ttk.Label(
            self.frame_cap_nhat_hsq,
            text="Excel cần cột: soto | sothua | loaidat | tenfile | sogcn",
            foreground="blue",
        ).grid(row=3, column=0, columnspan=6, sticky="w", padx=5, pady=(3, 0))

        for col_index in (1, 4):
            self.frame_cap_nhat_hsq.columnconfigure(col_index, weight=1)
    ...
    def set_mode_radios_state(self, state: str) -> None:
        widgets = [
            *self.mode_radios, self.chk_gcn, self.chk_hsq,
            self.ent_ngay_dk, self.ent_chu_id, self.cbo_loai_hsq,
            self.chk_day_hsq_capnhat, self.chk_dry_run_capnhat, self.ent_folder_upload,
            self.btn_browse_folder_upload,
        ]
        for widget in widgets:
            widget.config(state=state)

        # readonly combobox không nhận "normal", giữ lại readonly khi bật lại.
        if state == "normal":
            self.cbo_loai_hsq.config(state="readonly")
            self._on_toggle_day_hsq_capnhat()

    def _on_mode_change(self) -> None:
        mode = self.var_mode.get()
        if mode == MODE_CAP_NHAT_HSQ:
            if not self.frame_cap_nhat_hsq.winfo_ismapped():
                self.frame_cap_nhat_hsq.pack(fill="x", padx=10, pady=5, before=self.button_frame)
        else:
            self.frame_cap_nhat_hsq.pack_forget()

    def _on_toggle_day_hsq_capnhat(self) -> None:
        state = "normal" if self.var_day_hsq_capnhat.get() else "disabled"
    ...
    def chay(self) -> None:
        ...
        if mode == MODE_CAP_NHAT_HSQ:
            self._chay_cap_nhat_hsq(xa_id, input_file, output_file)
            return
    ...
    def _chay_cap_nhat_hsq(self, xa_id: str, input_file: str, output_file: str) -> None:
        ngay_dk = self.var_ngay_dk.get().strip()
        chu_id = self.var_chu_id.get().strip() or None
        day_hsq = self.var_day_hsq_capnhat.get()
        folder_upload = self.var_folder_upload.get().strip()
        loai_ho_so_quet = self.loai_hsq_display_to_id.get(self.var_loai_hsq.get(), 2)
        dry_run = self.var_dry_run_capnhat.get()

        if not ngay_dk:
            messagebox.showwarning("Thiếu thông tin", "Nhập ngày đăng ký lần đầu.")
            return
        ...
        try:
            parcels = doc_danh_sach_cap_nhat_hsq(input_file)
        except Exception as exc:
            messagebox.showerror("Lỗi đọc Excel", str(exc))
            return
        ...
        confirm_text = (
            f"File có {len(parcels)} dòng.\n\n"
            "Chế độ: CẬP NHẬT HỒ SƠ QUÉT (CHUACOGIAY → thật) + thông tin đăng ký/chủ.\n"
            ...
            "Nếu 1 thửa tìm thấy nhiều đơn đăng ký, TẤT CẢ đơn đó sẽ được cập nhật.\n\n"
            "Tiếp tục?"
        )
        if not messagebox.askyesno("Xác nhận Cập nhật hồ sơ quét", confirm_text):
            return
        ...
        threading.Thread(
            target=self._run_process_cap_nhat_hsq,
            args=(xa_id, parcels, output_file, day_hsq, chu_id, loai_ho_so_quet, folder_upload, ngay_dk),
            daemon=True,
        ).start()

    def _run_process_cap_nhat_hsq(
        self,
        xa_id: str,
        parcels: list[dict[str, Any]],
        output_file: str,
        ...
        writer = ExcelCapNhatHsqWriter(output_file)
        processed_parcels = 0
        total_results = 0
        thanh_cong = 0
        loi = 0
        ...
        try:
            self.log("=" * 70)
            self.log("BẮT ĐẦU - Chế độ: CẬP NHẬT HỒ SƠ QUÉT")
            self.log(f"Mã xã: {xa_id}")
            self.log(
                f"Đẩy HSQ: {day_hsq} | Loại HSQ: {loai_ho_so_quet} "
                f"({LOAI_HO_SO_QUET_OPTIONS.get(loai_ho_so_quet, '')})"
            )
            self.log(f"ID chủ: {chu_id or '(không đổi)'} | DRY_RUN: {DRY_RUN}")
            self.log(f"Số dòng đầu vào: {len(parcels)}")
            ...
            for parcel_index, parcel in enumerate(parcels, start=1):
                ...
                loai_dat = parcel["loaidat"]
                ten_file = parcel["tenfile"]
                so_gcn = parcel["sogcn"]
                ...
                if not so_to or not so_thua:
                    writer.append_result(
                        {
                            ...
                            "trang_thai": "Bỏ qua", "ghi_chu": "Thiếu số tờ hoặc số thửa.",
                        }
                    )
                    total_results += 1
                    bo_qua += 1
                    ...
                    try:
                        registrations = self.client.tim_tat_ca_don(
                            xa_id=xa_id, so_to=so_to, so_thua=so_thua
                        )
                    except Exception as exc:
                        writer.append_result(
                            {
                    ...
                    if not registrations:
                        ...
                        for registration in registrations:
                                ...
                                writer.append_result(
                                    {
                                        ...
                                        "trang_thai": "Lỗi",
                                        "ghi_chu": "tinhHinhDangKyId không hợp lệ.",
                                    }
                                )
                                total_results += 1
                                loi += 1
                            ...
                            row_result = xu_ly_mot_don_cap_nhat_hsq(
                                client=self.client,
                            ...
                            writer.append_result(row_result)
                            total_results += 1

                            trang_thai = row_result.get("trang_thai")
                            if trang_thai == "Thành công":
                                thanh_cong += 1
                            elif trang_thai == "Bỏ qua":
                                bo_qua += 1
                            ...
                            self.log(f"      {trang_thai}: {row_result.get('ghi_chu')}")
                ...
                if processed_parcels % SAVE_EVERY_PARCELS == 0:
                    writer.save()
                    self.log(f"💾 Đã tự lưu kết quả sau {processed_parcels} dòng.")

            writer.save()
            self.log("💾 Đã lưu file kết quả cuối cùng.")

            self.log("=" * 70)
            self.log(f"Đã xử lý dòng Excel: {processed_parcels}/{len(parcels)}")
            ...
            if self.stop_flag:
                self.set_status(
                    f"Đã dừng: xử lý {processed_parcels}/{len(parcels)} dòng, đã lưu kết quả"
                )
            else:
                self.set_status(f"Hoàn tất: {processed_parcels} dòng, {thanh_cong} thành công")
